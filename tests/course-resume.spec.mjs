...
test("コースを切り替えても各コースの位置を独立に保持する", async ({ page }) => {
  ...
  await expect(page.locator("#current-path")).toContainText("仮定法過去 ＞ 各論");
...
test("コース切替で回答・復習・修了記録を保持する", async ({ page }) => {
  ...
  const finalChecks = { subjunctive: { bestScore: 20, lastScore: 20, cleared: true, bestTotal: 27 } };
  await seedProgress(page, {
    courseId: "subjunctive",
    stage: 0,
    question: 0,
...
test("375pxでも概論CTAをキーボードで実行でき横にはみ出さない", async ({ page }) => {
  ...
  const action = page.getByRole("button", { name: "仮定法過去へ" });
  await action.focus();
  await expect(action).toBeFocused();
  await page.keyboard.press("Enter");
  await expect(page.locator("#current-path")).toContainText("仮定法過去");
